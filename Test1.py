...
"""
        ...
        log_lines.append(f"\033[1mIteration:\033[0m {i}")
        log_lines.append(f"\033[SE(3) end - effector config:\033[0m {T_theta}")
        log_lines.append(f"\033[error twist V_b:\033[0m {Vb}")
        log_lines.append(
            f"\033[Angular error magnitude ||omega_b||:\033[0m {np.linalg.norm([Vb[0], Vb[1], Vb[2]])}")
        log_lines.append(
            f"\033[Linear error magnitude ||v_b||:\033[0m {np.linalg.norm([Vb[3], Vb[4], Vb[5]])} \n")
